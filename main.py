...
print("Simplificador de Expressões Booleanas:")
print('\033[33m'+'Versão 2.1'+'\033[0;0m')
print()
inteiros = list(range(1,11))
for i in range(len(inteiros)):
    inteiros[i] = str(inteiros[i])
print('Digite um numero inteiro entre 1 e 10:')
totaldevariaveis = (input("Nº Variáveis: "))
while totaldevariaveis not in inteiros:
    print('\033[31m' + 'Sua entrada está invalida' + '\033[0;0m')
    totaldevariaveis = (input("Nº Variáveis: "))

totaldevariaveis = int(totaldevariaveis)
numerodepossibilidades = 2**totaldevariaveis
print('\033[32m'+"Vamos lá! voce selecionu %.i variáveis. \nDigite entre espaços as %.i saídas da tabela verdade." %(totaldevariaveis,numerodepossibilidades) )
possibilidades = binarios(numerodepossibilidades)
saidas = input('\033[0;0m'+"Saídas: ").split()
confirmacao = bool()
entradasvalidas = ["0","1","x","X"]
erro = 0
while any(digit not in entradasvalidas for digit in saidas) or len(saidas) != numerodepossibilidades:
    erro +=1
    if erro == 1:
        print('\033[31m'+"Sua entrada está invalida"+'\033[0;0m')
    elif erro == 2:
        print('\033[31m'+"Sua entrada continua invalida"+'\033[0;0m')
    else:
        print('\033[31m'+"Caso sua entrada permaneça invalida não avançaremos"+'\033[0;0m')
        erro = 0
    saidas = input("Saídas: ").split()
print()
...
while len(Implicantes) != 0:
    print("Implicantes (Ordem %.i):" % Interacao)
    for i in list(Implicantes.items()):
        print(i)
    print()
...
while len(minitermos) != 0:
    if len(EPI) == 0 and len(PI) != 0:
        ...
        for chave in ExtraidosPetrick:
            EPI[chave] = PI[chave]
        equacaofinal.update(EPI)
        print("Essenciais Implicantes Extraídos")
        for i in list(EPI.items()):
            print(i)
        print()
        break
    if len(PI) == 0 and len(EPI) == 0:
        print('\033[31m'+"Para simplificar a equação é necessário \nque todos os minitermos estejam presentes nos Primos Implicantes"+'\033[0;0m')
        if len(minitermos) == 1:
            print('\033[31m'+"O minitermo a seguir não está presente: \n"+'\033[0;0m')
        else:
            print('\033[31m' + "Os minitermos a seguir não estão presentes: \n" + '\033[0;0m')
        for i in minitermos:
            print('\033[31m'+i+'\033[0;0m', end= " ")
        print("\n")
        minitermos = []
        equacaofinal = {}
        break
    ...
    if len(PI) > 0:
        print("Primos Implicantes Reduzido (Interação %.i):" % Interacao)
        for i in list(PI.items()):
            print(i)
        print()
    EPI, PI, minitermos, ProvaveisPetrick = essencial_prime_implicant(minitermos, PI)
...
print("Expressão Bool. Simplificada \n")
equacaosimples = expressao_simplificada(equacaofinal, letras, saidas)
print('\033[33m'+equacaosimples+'\033[0;0m')
